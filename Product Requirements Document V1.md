...
* **Pain Points:** 
  ...
  * Struggles to frame accomplishments in the "Google XYZ" style (*Accomplished X, measured by Y, by doing Z*).
...
* **Structured Local Disk Auto-Save:** Automatically saves generated `.tex` files to local directories: `resumes/<company_name>/<job_role>/<resume_date>.tex` and stores generation metadata in a local history log file.
...
* **US8 - Google XYZ Formatting:** As a user, I want the tailoring engine to synthesize the selected Atomic Facts into high-impact Google-style XYZ bullet points that align directly with the JD's requirements.
...
* **US11 - Auto-Save to Disk:** As a user, when I generate a resume, I want the system to automatically save the completed `.tex` file to `resumes/<company_name>/<job_role>/<resume_date>.tex` on my local drive so that I have a persistent physical copy of every application.
...
2. **Dynamic Bullet Synthesizer:** Take selected Atomic Facts and construct them into full prose sentences applying the Google XYZ format (*Accomplished X, measured by Y, by doing Z*) tailored to the tone of the JD.
...
1. **Local Auto-Save:** The application must write files directly to the host machine's directory tree under `resumes/<company_name>/<job_role>/<resume_date>.tex`.
2. **History Ledger:** Save metadata (Company Name, Job Role, Timestamp, Path to `.tex` file) to a local JSON/YAML file to act as the generation log.
